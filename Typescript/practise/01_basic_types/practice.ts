...
// ------------------------------------------------------------
const nums = [1, 2, 3]; // 要求: readonly number[]
...
const coords = [120, 30]; // 要求: [number, number]
...
const entry = ['age', 30]; // 要求: readonly [string, number]
...
// ------------------------------------------------------------
const config = { port: 3000 }; // 要求: { readonly port: number }
// @ts-expect-error  port 只读，不能改（注解写对后这行才会报错被吃掉）
config.port = 8080;

// 第 8 题：嵌套只读对象。要求 host 和 port 都只读。
const server = { host: 'localhost', port: 3000 }; // 要求: { readonly host: string; readonly port: number }
